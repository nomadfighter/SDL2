...
void DrawTriangle(SDL_Renderer* renderer)
{
	...
	/*SDL_Rect viewport;
	SDL_RenderGetViewport(renderer, &viewport);*/
    SDL_SetRenderDrawColor(renderer, 255, (Uint8)0,
		(Uint8)0, (Uint8)0);
...
void DrawCircle(SDL_Renderer* renderer)
{
	...
	if (points.empty())
	{
		for (int i = 0; i < cnt; ++i)
		{
			float delta = 360.0f / cnt;
			float rad = (float)(i * delta * M_PI) / 180.0f;
			SDL_Point pt;
			pt.x = 100 * SDL_cosf(rad);
			pt.y = 100 * SDL_sinf(rad);
			points.push_back(pt);
		}
		SDL_Point st = points[0];
		points.push_back(st);
	}
	std::vector<SDL_Point> tmp(points);
	for (auto& pt : tmp)
	{
		pt.x = pt.x * radius / 100 + x;
		pt.y = pt.y * radius / 100 + y;
	}
	SDL_RenderDrawLines(renderer, tmp.data(), tmp.size());
	tmp.shrink_to_fit();
}
